...
//  ToDoTask (RealmApp)
//
//  Created by Алексей Синяговский on 22.04.2022.
//
...
class TasksViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let doneEffect = indexPath.section == 0 ? "Done" : "Undone"
        let isComplete = indexPath.section == 0 ? true : false
        let newSection = IndexPath(row: 0, section: indexPath.section == 0 ? 1 : 0)
        
        let doneAction = UIContextualAction(style: .normal, title: doneEffect) { _, _, isDone in
